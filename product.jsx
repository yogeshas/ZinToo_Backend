...
export default function ProductDetailPage() {
  ...
  const handleAddToCart = async () => {
    ...
    // Validate color selection if product has color
    ...
    console.log('🛒 [PRODUCT] Adding to cart:', product.id, 'quantity:', quantity, 'size:', selectedSize, 'color:', selectedColor);
    const success = await addToCartBackend(product.id, quantity, selectedSize, selectedColor);
  ...
  const getMaxQuantity = () => {
    if (!product || !selectedSize || !product.sizes) return 10; // Default max quantity
    const sizeStock = product.sizes[selectedSize];
    return sizeStock > 0 ? Math.min(sizeStock, 10) : 0; // Cap at 10, but respect available stock
  };
              ...
              <div className="text-white">
                ...
                {hasDiscount && (
                  <div className="inline-block bg-red-600 text-white px-4 py-2 rounded-full text-sm font-semibold">
                    Save ₹{product.original_price! - displayPrice}
                  </div>
                )}
              </div>
            </div>
            ...
            {product.color && (
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-3">
                  {(() => {
                    // Parse colors - handle both single color and comma-separated colors
                    const colors = product.color.includes(',') 
                ...
                {selectedColor && (
                  <div className="flex items-center space-x-2 mt-2">
                    <div className="w-4 h-4 rounded-full border-2 border-purple-600" style={{ backgroundColor: selectedColor }}></div>
              ...
              {selectedSize && (
                <div className="bg-blue-50 p-3 rounded-lg">
                  <p className="text-sm text-blue-700 font-medium">
                    Max quantity: <span className="font-bold">{getMaxQuantity()}</span> (based on {selectedSize} availability)
                </p>
                </div>
              )}
            </div>
