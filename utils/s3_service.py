...
class S3Service:
    ...
    def upload_file(self, file, file_id, file_type="image", folder="category"):
        """
        ...
        Args:
            ...
            folder: S3 folder name (category, review_media, etc.)
    ...
    def upload_product_file(self, file, product_id, color_name=None, file_type="image"):
        """
        Upload product file to S3 with organized folder structure
        ...
        try:
            ...
            # Create organized folder structure: product/product_id/color_name/filename
            if color_name:
                # Color-specific image
                folder_path = f"product/{product_id}/{color_name}/"
            else:
                # Main product image
                folder_path = f"product/{product_id}/main/"
    ...
    def upload_multiple_files(self, files, file_id, file_type="media", folder="review_media"):
        """
        Upload multiple files to S3 and return the public URLs
        
        Args:
            files: List of file objects from request
            file_id: ID of the review/category/etc.
            file_type: Type of file (image, video, media, etc.)
        ...
        for file in files:
            ...
            # Upload file
            s3_url = self.upload_file(file, file_id, file_type_actual, folder)
            if s3_url:
