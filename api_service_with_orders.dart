...
class ApiService {
  ...
  Future<Map<String, dynamic>> getOnboardingStatus(String authToken) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // User doesn't have onboarding record yet (new user)
  ...
  Future<Map<String, dynamic>> getOrders(String authToken, {String? status}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return {
          'success': true,
          'orders': responseData['encrypted_data'] != null 
              ? _decryptData(responseData['encrypted_data'])['orders'] 
              : [],
        };
      } else {
        final errorData = jsonDecode(response.body);
  ...
  Future<Map<String, dynamic>> getOrderDetails(String authToken, int orderId) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return {
          'success': true,
          'order': responseData['encrypted_data'] != null 
              ? _decryptData(responseData['encrypted_data'])['order'] 
              : null,
        };
      } else {
        final errorData = jsonDecode(response.body);
  ...
  Future<Map<String, dynamic>> approveOrder(String authToken, int orderId) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return {
          'success': true,
          'message': responseData['encrypted_data'] != null 
              ? _decryptData(responseData['encrypted_data'])['message'] 
  ...
  /// Decrypt data using the same method as your backend
  Map<String, dynamic> _decryptData(String encryptedData) {
    try {
      // Get the crypto secret from environment (same as backend)
      final cryptoSecret = dotenv.env['CRYPTO_SECRET'] ?? 'my_super_secret_key_32chars!!';
      
      // Create the key and IV (same as backend)
      final key = Key.fromBase64(base64.encode(utf8.encode(cryptoSecret.padRight(32).substring(0, 32))));
      final iv = IV.fromBase64(base64.encode(utf8.encode('1234567890123456')));
      ...
      // Fallback: try to decode as plain JSON (in case encryption is disabled)
      try {
        return jsonDecode(encryptedData);
      } catch (e2) {
        print('Error parsing as plain JSON: $e2');
        return {};
      }
    }
  }
