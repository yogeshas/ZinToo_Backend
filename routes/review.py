...
def upload_media(current_customer):
    ...
    base_dir = os.path.join(current_app.root_path, "assets", "img", "review_media")
    ...
    for f in files:
        ...
        if ext in image_ext:
            ...
            f.save(save_path)
            image_urls.append(f"/api/reviews/media/{unique_name}")
        elif ext in video_ext:
            if not (f.mimetype or "").startswith("video/"):
                return jsonify({"success": False, "error": "Invalid video file"}), 400
            # Optional: enforce size limits via content_length
            f.save(save_path)
            video_urls.append(f"/api/reviews/media/{unique_name}")
        else:
            return jsonify({"success": False, "error": f"Unsupported file type: {ext}"}), 400

    enc = encrypt_payload({"images": image_urls, "videos": video_urls})
    return jsonify({"success": True, "encrypted_data": enc})


@review_bp.route("/media/<path:filename>", methods=["GET"])
